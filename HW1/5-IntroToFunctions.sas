...
data datefun2;
		my_birthday = mdy(10, 29, 1998);
		days_alive = datdif(my_birthday, today(), 'act/act');
		years_alive = yrdif(my_birthday, today(), 'act/act');
		week_day = weekday(my_birthday);
		
run;

proc print data = datefun2;
		title "Output 3";
run;
